...
def RFBModuleB2(x, in_channels):
    ...
    with tf.variable_scope('branch1'):
        conv1x1=slim.conv2d(x, 2*inc, (1, 1), stride=1, scope='conv1x1')
        branch1_conv3x3=depthwise_conv(conv1x1, kernel=3, stride=1, padding='SAME',activation_fn=tf.nn.relu)
        branch1_conv3x3_dilation=slim.conv2d(branch1_conv3x3, 2*inc, (3, 3), stride=1, padding='SAME',rate=2, activation_fn=None)
    with tf.variable_scope('branch2'):
        conv1x1=slim.conv2d(x, 2*inc, (1, 1), stride=1, scope='conv1x1')
        branch2_conv5x5_1=depthwise_conv(conv1x1, kernel=3, stride=1, padding='SAME',activation_fn=tf.nn.relu) 
    ...
    x = tf.transpose(x, [0, 1, 2, 4, 3])
    x = tf.reshape(x, [batch_size, height, width, 8*inc])
